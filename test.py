...
def compare_simulation(filename):
    with open(os.path.join(TEST_DIRECTORY, "test_levels", f"{filename}.json")) as f:
        level = json.load(f)
    with open(os.path.join(TEST_DIRECTORY, "test_inputs", f"{filename}.json")) as f:
        inputs = json.load(f)
...
def compare_simulations(filenames):
    prev_games = []
    prev_level = []
    expected = []
    for filename in filenames:
        ...
        # create starting games
        expected.append((inputs, outputs, filename))
        game = lab.make_new_game(copy.deepcopy(level))
        prev_games.append(game)
        prev_level.append(level)

    max_moves = max(len(tup[0]) for tup in expected)

    # now go round robin, take one step for each game before moving onto the next step
    for i in range(max_moves):
        next_games = []
        next_level = []
        for exp, game, level in zip(expected, prev_games, prev_level):
            (inputs, outputs, fname) = exp
            if i >= len(inputs):
                next_games.append(None)
                next_level.append(None)
                continue  # no moves left in this test, continue with others

            direction = inputs[i]
            exp_dump, exp_win = outputs[i]

            # make the same move twice from the input game, should get same results
            new_game = lab.step_game(game, direction)
            new_game2 = lab.step_game(game, direction)
            assert new_game == new_game2, "be careful not to modify the input game!"
            next_games.append(new_game)
            next_level.append(exp_dump)

            # check that the new_game is correct
            game_dump = lab.dump_game(new_game)
            err_msg = compare_boards(game_dump, exp_dump)
            if err_msg is not None:
                assert False, f"Unexpected results in step {i} for {fname}, moving {direction} starting from the following board ({err_msg}):\n\n{level}\n\nYou can copy/paste this JSON representation into the GUI to test."
            if level != exp_dump:
                # something moved, check that game representation changed
                assert new_game != game, f"Unexpected results in step {i} for {fname}, expected player moving to result in change in game representation but saw none"
            assert lab.victory_check(new_game) == exp_win, f"Incorrect victory check in step {i} for {fname} the following board (expected {exp_win}):\n\n{new_level}\n\nYou can copy/paste this JSON representation into the GUI to test."

        # update games and levels for next round
...
def test_random_multiple_games():
    for test_group in range(10):
        compare_simulations(['random_%04d' % (test_group*10 + i)  for i in range(10)])
...
def test_solver(test_group):
    ...
    for puzzle, elen in zip(SOLVER_TEST_GROUPS[test_group], SOLUTION_LENGTHS[test_group]):
        ...
        if elen is None:
            ...
            assert isinstance(result, list), f"Expected a list containing the solution for {puzzle}, got {result}."
